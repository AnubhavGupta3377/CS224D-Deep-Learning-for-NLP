...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    label_size = self.config.label_size
    with tf.variable_scope('HiddenLayer', initializer=xavier_weight_init()) as scope:
      W1 = tf.get_variable('weights', [input_size, hidden_size])
      b1 = tf.get_variable('biases', [hidden_size])
      z2 = tf.matmul(window, W1) + b1
      a2 = tf.nn.tanh(z2)
      if self.config.l2:
        tf.add_to_collection('total_loss', 0.5 * self.config.l2 * tf.nn.l2_loss(W1))
        
    with tf.variable_scope('OutputLayer', initializer=xavier_weight_init()) as scope:
      W2 = tf.get_variable('weights', [hidden_size, label_size])
      b2 = tf.get_variable('biases', [label_size])
      z3 = tf.matmul(a2, W2) + b2
      if self.config.l2:
        tf.add_to_collection('total_loss', 0.5 * self.config.l2 * tf.nn.l2_loss(W2))
    output = tf.nn.dropout(z3, self.dropout_placeholder)
    ### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    ce_loss_vec = tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)
    cross_entropy = tf.reduce_mean(ce_loss_vec)
