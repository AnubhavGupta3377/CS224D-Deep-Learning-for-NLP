...
class Config(object):
  ...
  embed_size = 50
  hidden_size = 100
  num_steps = 10  # Number of time steps to Backpropagate
  max_epochs = 16
  early_stopping = 2
  dropout = 0.9
  lr = 0.001
...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      num_steps = self.config.num_steps
      L = tf.get_variable('Embedding', [vocab_size,embed_size], trainable=True)
      embed = tf.nn.embedding_lookup(L, self.input_placeholder)
      inputs = [tf.squeeze(x, [1]) for x in tf.split(1, num_steps, embed)]
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope('ProjectinoLayer') as scope:
      hidden_size = self.config.hidden_size
      vocab_size = len(self.vocab)
      U = tf.get_variable('Weights', [hidden_size,vocab_size])
      b2 = tf.get_variable('bias', [vocab_size])
      outputs = [(tf.matmul(o,U) + b2) for o in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    embed_size = self.config.embed_size
    rnn_outputs = []
    with tf.variable_scope('RNNHiddenLayer') as scope:
      self.initial_state = tf.zeros([batch_size,hidden_size])
      H = tf.get_variable('Weights', [hidden_size,hidden_size])
      I = tf.get_variable('Representation', [embed_size, hidden_size])
      b1 = tf.get_variable('bias', [hidden_size])
      for t, e in enumerate(inputs):
        e = tf.nn.dropout(e, self.dropout_placeholder)
        if len(rnn_outputs) == 0:
          h = self.initial_state
        h = tf.nn.sigmoid(tf.matmul(h,H) + tf.matmul(e,I) + b1)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ...
    state, y_pred = session.run(
        [model.final_state, model.predictions[-1]], feed_dict=feed_dict)
    ### END YOUR CODE
...
def test_RNNLM():
  ...
  with tf.Session() as session:
    ...
    session.run(init)
    # Highlight below code to load saved model
    ###########################################################
