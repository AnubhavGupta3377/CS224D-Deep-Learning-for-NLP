...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.expand_dims(tf.gather(L,word_idx), 0) # Get the embedding of the word and reshape it to desired shape
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            left_child_tensor = node_tensors[node.left]
            right_child_tensor = node_tensors[node.right]
            children_tensor = tf.concat([left_child_tensor, right_child_tensor], axis=1)
            z = tf.matmul(children_tensor, W1) + b1
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope('Projection', reuse=True):
            U = tf.get_variable('U')
        ce_loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=tf.constant(labels)))
        reg = self.config.l2 * (tf.nn.l2_loss(W) + tf.nn.l2_loss(U))
        loss = ce_loss + reg
        # END YOUR CODE
        return loss
    ...
    def train(self, verbose=True):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            if epoch_loss>prev_epoch_loss*self.config.anneal_threshold:
                ...
                print ('annealed lr to {}f'.format(self.config.lr))
